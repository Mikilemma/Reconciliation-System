...
const isSettlementProcessing = ref<boolean>(false);
const settlementPhase = ref<number>(0);
...
const CROSS_SESSION_PREF_KEY = 'p2p.enableCrossSessionMatching';
...
const crossSessionStatusText = computed(() => {
  if (!enableCrossSessionMatching.value) {
    return 'Cross-session matching is disabled for this upload.';
  }
  if (selectedFiles.value.length > 1) {
    return 'Cross-session matching is enabled. For multi-file uploads, matching runs on SWITCH files.';
  }
  return 'Cross-session matching is enabled for this upload.';
});
...
const getPreviewRecords = (response: any): any[] => {
  const candidate =
    response?.records ??
    response?.parsedData ??
  ...
  if (candidate === null || candidate === undefined || candidate === 'null') return [];
  ...
  try {
    ...
    if (enableCrossSessionMatching.value && selectedFiles.value.length === 1) {
      ...
      for (let i = 0; i < selectedFiles.value.length; i++) {
        ...
        uploadStatus.value = `Uploading file ${i + 1} of ${selectedFiles.value.length}...`;
        ...
        const response = await axios.post('/api/p2p/files/upload', formData, {
          ...
          onUploadProgress: (progressEvent) => {
            if (progressEvent.total) {
              ...
              const fileContribution = (fileProgress / selectedFiles.value.length);
              uploadProgress.value = Math.round(baseProgress + fileContribution);
            }
          }
        });
      ...
      const combinedData = allResponses.reduce((acc, response, index) => {
        ...
        const records = getPreviewRecords(response);
      ...
      uploadStatus.value = '✅ All files uploaded & parsed successfully! Ready for settlement processing.';
      console.log('Upload complete. No auto-matching (no SWITCH files detected). User must click "Start Settlement Process".');
      
      // Log file types for debugging
      const fileTypes = combinedData.files.map((f: any) => f.fileType);
      console.log('Uploaded file types:', fileTypes);
      
      // Only show "matching" if SWITCH files present (where cross-session actually runs)
      const hasSwitchFiles = combinedData.files.some((f: any) => f.fileType?.toUpperCase().includes('SWITCH'));
      if (enableCrossSessionMatching.value && hasSwitchFiles) {
        uploadStatus.value = 'Performing cross-session matching on SWITCH files...';
        try {
          // ... existing matching code ...
          uploadStatus.value = `✅ Cross-session matching completed!`;
        } catch (error) {
          console.error('Cross-session matching error:', error);
          uploadStatus.value += ' (Matching failed - click Start Process to continue)';
        }
      }
    }
...
const confirmAndSave = async () => {
  ...
  isSettlementProcessing.value = true;
  settlementPhase.value = 0;
  isLoading.value = true;
  uploadError.value = '';
  uploadStatus.value = 'Validating uploaded files...';
  uploadProgress.value = 5;

  try {
    const files = parsedData.value.files || [];

    // Ensure we always include the uploaded file session IDs. Files with 0 records may still be required.
    const fileSessionIds = files
      .map((f: any) => f.sessionId)
      .filter((id: string | undefined) => !!id);

    if (fileSessionIds.length === 0) {
      uploadError.value = 'No uploaded files were found. Please re-upload and try again.';
      console.error('confirmAndSave: no fileSessionIds (parsedData):', parsedData.value);
      return;
    }

    // Settlement date is primarily informational; if the backend needs it, it will default internally.
    const settlementDate = new Date().toISOString().split('T')[0];

    console.debug('confirmAndSave payload', { settlementDate, fileSessionIds });
    
    settlementPhase.value = 1;
    ...
    settlementPhase.value = 2;
    uploadStatus.value = 'Performing reconciliation...';
    uploadProgress.value = 45;
    await axios.post(`/api/p2p/reconciliation/${session.id}/perform`, {}, { timeout: 300000 });
    
    settlementPhase.value = 3;
    uploadStatus.value = 'Loading results...';
    uploadProgress.value = 70;
    await loadTransactions(session.id);
    
    settlementPhase.value = 4;
    uploadStatus.value = 'Generating summary...';
    ...
    const errorMsg = data?.message || data?.error || error.message || 'Unknown error';
    uploadError.value = `Save failed (${status ?? 'N/A'}): ${errorMsg}`;
    console.error('Confirm and save failed:', error);
    console.error('Confirm and save response data:', data);
  } finally {
    isSettlementProcessing.value = false;
    settlementPhase.value = 0;
    isLoading.value = false;
  }
};
...
const formatRecordPreview = (records: unknown): string => {
  ...
  if (!records || records === 'null' || (Array.isArray(records) && records.length === 0)) return 'No records available';
  try {
...
onMounted(() => {
  const savedPreference = localStorage.getItem(CROSS_SESSION_PREF_KEY);
  if (savedPreference !== null) {
    enableCrossSessionMatching.value = savedPreference === 'true';
  }
  loadSessions();
});

watch(enableCrossSessionMatching, (isEnabled) => {
  localStorage.setItem(CROSS_SESSION_PREF_KEY, String(isEnabled));
});
...
<template>
  <div class="container mx-auto p-6 max-w-6xl">
    ...
    <div v-if="currentStep === 1" class="animate-slide-up">
      <Card class="banking-card-elevated mb-6 border-0 shadow-none bg-transparent">
        <CardHeader>
        ...
        <CardContent>
          ...
          <div class="mb-6 p-4 rounded-xl bg-primary/5 border border-primary/20">
                ...
                <div class="text-left">
                  <div class="flex items-center gap-2">
                    ...
                    <span
                      ...
                      :class="enableCrossSessionMatching
                        ? 'bg-success/10 text-success border-success/30'
                        : 'bg-muted text-muted-foreground border-border'"
                    >
                      {{ enableCrossSessionMatching ? 'Enabled' : 'Disabled' }}
                    </span>
                  </div>
            ...
            <div class="mt-2 text-xs text-muted-foreground">{{ crossSessionStatusText }}</div>
          </div>
          ...
          <div v-if="isLoading" class="mb-8">
            <div class="flex justify-between text-sm mb-3 font-semibold">
              <span class="flex items-center gap-2">
                ...
                {{ uploadStatus || (isSettlementProcessing ? 'Processing settlement...' : 'Processing files...') }}
              </span>
              <span v-if="uploadProgress > 0" class="text-primary">{{ uploadProgress }}%</span>
            </div>
            <div class="w-full bg-muted rounded-full h-3 overflow-hidden shadow-inner border border-border">
              <div class="bg-primary h-full rounded-full transition-all duration-700 ease-out shadow-lg shadow-primary/50" :style="{ width: uploadProgress + '%' }" />
            </div>
            <div v-if="isSettlementProcessing" class="mt-4 space-y-2 rounded-xl border border-border/60 bg-muted/20 p-3">
              <div
                v-for="(phaseLabel, idx) in settlementPhases"
                :key="phaseLabel"
                class="flex items-center justify-between text-xs"
              >
                <div class="flex items-center gap-2">
                  <CheckCircle
                    v-if="idx < settlementPhase"
                    class="h-4 w-4 text-success"
                  />
                  <RefreshCw
                    v-else-if="idx === settlementPhase"
                  ...
                  ></div>
                  <span :class="idx <= settlementPhase ? 'text-foreground font-semibold' : 'text-muted-foreground'">
                    {{ phaseLabel }}
                  </span>
                </div>
                <span v-if="idx < settlementPhase" class="text-success font-semibold">Done</span>
                <span v-else-if="idx === settlementPhase" class="text-primary font-semibold">Running</span>
    ...
    <div v-if="currentStep === 3" class="animate-slide-up">
      ...
      <!-- Results Table -->
      <Card class="banking-card-elevated overflow-hidden border-0 shadow-none bg-transparent mb-8">
